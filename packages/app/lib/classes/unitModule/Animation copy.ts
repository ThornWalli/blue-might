...
> {
  ...
  setAnimationAction(type: string, duration?: number) {
    if (!this.mixer) {
    ...
    console.log('Set animation action:', type);
    this.action = type;
    this.fadeToAction(this.mixer ? this.actions : {}, type, duration, duration);
  ...
  fadeToAction(
    ...
    if (this.activeAction) {
      ...
      this.activeAction.fadeOut(fadeOutDuration);
    }
